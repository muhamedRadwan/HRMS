    ...
    public function givePermissions( $formId, $request){
        ...
        foreach($roles as $role){
            if(isset($request[ '_role_' . $role->name ])){
                $role->attachPermission('browse bread ' . $formId);
                if(isset($request['read'])){
                    $role->attachPermission('read bread ' . $formId);
                }
                if(isset($request['edit'])){
                    $role->attachPermission('edit bread ' . $formId);
                }
                if(isset($request['add'])){
                    $role->attachPermission('add bread ' . $formId);
                }
                if(isset($request['delete'])){ 
                    $role->attachPermission('delete bread ' . $formId);
                }
            }
        }
    }
