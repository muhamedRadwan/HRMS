...
@section('style')
<link href=" {{ URL::asset('/qr_login/css/style.css') }}" rel="stylesheet">
@stop
@section('content')
<div class="container" id="QR-Code">
         ...
         @else
            <h1>Hallo! {{Auth::getUser()->first_name}}</h1>
          @endif
...
@if( !Auth::getUser())
@section('scripts')
<script type="text/javascript" src=" {{ URL::asset('/qr_login/js/filereader.js') }}"></script>
<script type="text/javascript" src=" {{ URL::asset('/qr_login/js/qrcodelib.js') }}"></script>
<script type="text/javascript" src="{{ URL::asset('/qr_login/js/webcodecamjs.js ') }}"></script>
...
<script>
 function CallAjaxLoginQr(code) {
      $.ajax({
            type: "GET",
            cache: false,
            url : "{{route('attendance.guestAttendance','/')}}+code",
            data: {data:code},
